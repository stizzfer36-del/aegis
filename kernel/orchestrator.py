...
def estimate_tokens(text: str) -> int:
    ...
    if not cleaned:
        return 0
    cjk_chars = sum(1 for char in cleaned if unicodedata.east_asian_width(char) in ("W", "F"))
    non_cjk_chars = len(cleaned) - cjk_chars
...
class Orchestrator:
    ...
    def _with_policy_state(self, event: AegisEvent) -> AegisEvent:
        ...
        return replace(event, policy_state=state)
